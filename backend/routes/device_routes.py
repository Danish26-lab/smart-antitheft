...
def agent_register():
    ...
    try:
        ...
        # Check if agent provided a preferred device_id
        preferred_device_id = data.get('preferred_device_id')
        
        if preferred_device_id:
            # Use preferred device_id if it doesn't exist or is unowned
            existing = Device.query.filter_by(device_id=preferred_device_id).first()
            if existing:
                if existing.fingerprint_hash == fingerprint_hash:
                    # Same device, same fingerprint - use existing
                    device_id = preferred_device_id
                    logging.info(f"Using existing device with preferred ID: {device_id}")
                else:
                    # Different fingerprint - generate new ID
                    logging.warning(f"Preferred device_id {preferred_device_id} exists with different fingerprint, generating new ID")
                    preferred_device_id = None
            else:
                # Preferred ID doesn't exist - use it
                device_id = preferred_device_id
                logging.info(f"Using preferred device_id: {device_id}")
        
        if not preferred_device_id or Device.query.filter_by(device_id=preferred_device_id).first():
